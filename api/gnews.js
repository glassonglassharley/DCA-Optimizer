...
export default async function handler(req, res) {
    ...
    if (!sym) return res.status(400).json({ error: 'symbol required', handler: 'gnews' });
    ...
    try {
        ...
        return res.status(200).json({ articles, sym, handler: 'gnews' });
    } catch (e) {
        console.error(`[gnews] ${sym} error:`, e.message);
        return res.status(200).json({ articles: [], handler: 'gnews' });
    }
}
